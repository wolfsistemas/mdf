...
function boardPage(doc, project, settings, board, pageW, pageH, margin) {
  ...
  doc.setTextColor(90, 78, 66)
  doc.text(
    `${board.sheetName || settings.sheetName || 'MDF'} · ${board.sheetWidth} × ${board.sheetHeight} × ${board.thickness || settings.sheetThickness} mm · aproveitamento ${board.efficiency.toFixed(1)}% · ${board.placements.length} peças · modo ${board.mode === 'guillotine' ? 'serra / guilhotina' : board.mode === 'mac' ? 'MAC' : board.mode === 'manual' ? 'manual' : 'nesting livre'}`,
    margin,
    24
  )
